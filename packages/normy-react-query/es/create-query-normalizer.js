...
var cleanIds = function cleanIds(ids) {
...
export var createQueryNormalizer = function createQueryNormalizer(queryClient, normalizerConfig) {
  ...
  return {
    ...
    getQueryKeysById: function getQueryKeysById(ids) {
      return normalizer.getQueriesById(cleanIds(ids)).map(function (key) {
        return JSON.parse(key);
      });
    },
    invalidateQueriesById: function invalidateQueriesById(ids) {
      normalizer.getQueriesById(cleanIds(ids)).map(function (key) {
        return JSON.parse(key);
